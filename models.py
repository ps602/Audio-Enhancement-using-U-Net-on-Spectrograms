...
class UNetWithAttention(nn.Module):
    def __init__(self):
        ...
        self.attention_block1 = ChannelAttentionBlock(128)
    ...
    def forward(self, x):
        ...
        # Apply attention
        att1 = self.attention_block1(enc2)
        
        # Decoder
        dec1 = self.upconv1(att1)
        dec1 = self.match_size(dec1, enc1)
    ...
    def match_size(self, tensor, reference):
        if tensor.size()[2:] != reference.size()[2:]:
            tensor = F.interpolate(tensor, size=reference.size()[2:], mode='nearest')
        return tensor
